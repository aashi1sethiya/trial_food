...
st.set_page_config(page_title="Food Impact", page_icon=":egg:", layout="wide")
...
def sidebar(df):
    ...
    menu_item_name = st.sidebar.selectbox(
            ...
            options=[''] + list(df.query("MenuItemType == @menu_item_type")["MenuItemName"].unique()), # add default option as empty string
            index=0
    )
...
def get_carbon_footprint_for_dish_ingredients(df_selection):
    ...
    """
    labels = list(zip(df_selection['RawIngredientsEngSimple'].values[0], df_selection['RawIngredientsChinese'].values[0]))
    values = df_selection['CarbonFootprint'].values[0]
    descending_ix = np.array(values).argsort()[::-1] # sort values from largest to smallest
...
def main_page(df_selection):
    ...
    with col1: 
        lottie_welcome = lottie.load_lottiefile("./lottiefiles/walking-avocado.json")  # replace link to local lottie file
        st_lottie(lottie_welcome, speed=1, reverse=False, loop=True, quality='low', height=150, width=200, key='welcome')
    with col2:
        st.write("\n")
        st.write("\n")
        st.markdown("<h1 style='text-align: left; color: black;'> Our Food | Our Climate | Our Health </h1>", unsafe_allow_html=True)
    st.markdown("---")
    ...
    if len(df_selection) == 0:
        ...
        st.title(f":egg: Food Impact on ...")
        st.markdown("##")
        st.subheader(f"{df_selection['MenuItemName'].values[0]}\n")
        st.markdown("##")
        ...
        with col1:
            st.text("Ingredients: Carbon pie (whole recipe)\n")
        ...
        with col2:
            st.text("Carbon Footprint per 100g (kg CO2e / 100g):\n")
            value_per_100g, value_per_recipe, nServings = get_carbon_label_for_dish_per_100g(df_selection)
            fig_carbon_label_dish = plots.gauge_chart_carbon(value_per_100g, value_per_recipe, nServings)
        ...
        calories, carb, fat, protein = get_nutrition_label_for_dish_per_100g(df_selection)
        df_rdi = pd.read_csv('./data/nutrition_rdi.csv')
        
        col1, col2 = st.columns(2)
        with col1:
            fig_calories_per_100g = plots.donut_chart_nutrition(nutrient_value=calories, 
        ...
        with col2: 
            fig_carb_per_100g = plots.donut_chart_nutrition(nutrient_value=carb, 
        ...
        col1, col2 = st.columns(2)
        with col1:
            fig_fat_per_100g = plots.donut_chart_nutrition(nutrient_value=fat, 
        ...
        with col2: 
            fig_protein_per_100g = plots.donut_chart_nutrition(nutrient_value=protein, 
